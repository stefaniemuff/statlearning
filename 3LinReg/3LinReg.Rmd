...
To illustrate this point further, fit again the bodyfat example, but each time with only half of the data (randomly selected points each time). See how the model fit varies:
...
$\rightarrow$ Even if we could predict $\hat{Y}=f(X)$, the _true_ value $Y$ varies around $\hat{Y}$. We can compute a _\textcolor{red}{prediction interval}_ for new observations $Y$.
