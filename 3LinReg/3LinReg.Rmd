...
* @ISL: An Introduction to Statistical Learning. Chapter 3.  
...
* **Problem**: $\sigma$ is usually not known, but needs to be estimated.
...
we can create confidence intervals for $\beta_j$ in the same manner as we did for simple linear regression. 
...
This leads to the \textcolor{red}{confidence interval}. 
...
Even if we could predict $\hat{Y}=f(X)$, the _true_ value $Y$ varies around $\hat{Y}$. We can compute a \textcolor{red}{prediction interval} for new observations $Y$.
...
To answer this question, we have to \textcolor{red}{consider not only the uncertainty in the predicted value} $\hat y_0 =  \hat\beta_0 + \hat\beta_1 x_1^{(0)} + \ldots \hat\beta_p x_p^{(0)}$, but also the \textcolor{red}{irreducible error} $\varepsilon_i \sim N(0,\sigma^2)$.  


\vspace{4mm}

$\rightarrow$ The *prediction interval is always wider than the confidence range*.
...
The \textcolor{red}{Tukey-Anscombe} diagram plots the residuals against the fitted values. For the bodyfat data it looks like this:
...
* In simple regression, the leverage of individual $i$ is defined as $H_{ii} = (1/n) + (x_i-\overline{x})^2 \sum(x_i-\overline{x})^2$. 
...
* Leverage is defined as the diagonal elements of the hat matrix, i.e., the leverage of the $i$-th data point is $h_{ii}$ on the diagonal of $\mathbf{H = X(X^TX)^{-1}X^T}$. 

* A large leverage indicated that the observation ($i$) has a large influence on the estimation results, and that the covariate values ($\bf{x}_i$) are unusual.
...
\vspace{6mm}
However:  this model is \textcolor{red}{not identifiable}.\footnote{What does that mean? I could add a constant to $\beta_1, \beta_2, ...\beta_k$ and subtract it from $\beta_0$, and the model would fit equally well to the data, so it cannot be decided which set of the parameters is best.}
...
Please note that \textcolor{red}{a factor covariate with $k$ factor levels requires $k-1$ parameters!}
\vspace{2mm}

$\rightarrow$ The \textcolor{red}{degrees of freedom} of the model are also reduced by $k-1$.
...
The hierchical principle states that if we include an interaction in a model, we
...
The word _linear_ refers to the \textcolor{red}{linearity in the coefficients}, and not on a linear relationship between $Y$ and $X_1, \ldots , X_p$!
