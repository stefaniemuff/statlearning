...
\vspace{2mm} Again, the \(F\)-test can be used, but now \(F\) is
calculated like
...
\item
  ...
  as we did for simple linear regression (see slide 43). For example,
...
\begin{center}\includegraphics[width=0.5\linewidth]{3LinReg_files/figure-beamer/unnamed-chunk-24-1} \end{center}
...
\textbf{Where does a future observation lie with a certain confidence
(i.e., 95\%)?}
...
\item
  The expected value of \(\varepsilon_i\) is 0:
  \(\text{E}(\varepsilon_i)=0\). \vspace{1mm}
\item
  All \(\varepsilon_i\) have the same variance:
  \(\text{Var}(\varepsilon_i)=\sigma^2\).\\
  \vspace{1mm}
...
\begin{center}\includegraphics[width=0.5\linewidth]{3LinReg_files/figure-beamer/unnamed-chunk-27-1} \end{center}
...
\begin{center}\includegraphics[width=0.9\linewidth]{3LinReg_files/figure-beamer/unnamed-chunk-29-1} \end{center}
...
\vspace{2mm} \(\rightarrow\) To make the \(e_i\) more ``like
\(\epsilon_i\)'', we use \emph{\textcolor{red}{standardized}} or
...
\item
  ...
  which leads to a \textbf{quadratic} or \textbf{polynomial} regression
  (if higher order terms are used).
\end{itemize}
