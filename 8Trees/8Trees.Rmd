...
**A**: The Gini index and Entropy are _measure of impurity_. Moreover, they are differentiable (preferred for numerical optimization!).

```{r purity, echo=FALSE, fig.width=6, fig.height=5,fig.align = "center",out.width='70%'}
...
Remark: the deviance is a scaled version of the cross entropy: $-2\sum_{k=1}^K n_{jk} \log\hat{p}_{jk}$ where $\hat{p}_{jk}=\frac{n_{jk}}{N_j}$, thus `split="deviance"` implies that we split according to the entropy criterion.

---

```{r headInjury, echo=TRUE, fig.width=6, fig.height=5,fig.align = "center",out.width='70%'}
plot(tree.HIClass,type="proportional")
text(tree.HIClass,pretty=1)
```

Note: With `type="proportional" `, the length of branches are now proportional to the decrease in impurity.
...
\footnotesize
```{r headInjury2, echo=TRUE, fig.width=8, fig.height=6,fig.align = "center",out.width='85%'}
tree.HIClassG=tree(brain.injury~.,headInjury2,
                  subset=train,split="gini")
plot(tree.HIClassG)
