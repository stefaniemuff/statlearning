...
(Make sure you have access to the slides while you are in the breakout room).
...
* The number of trees, $B$, is not a tuning parameter, and the best is to choose it large enough (as large as "necessary"). An increase in $B$ will not lead to overfitting, and $B$ is not regarded as a tuning parameter. 
...
* To find out which number $B$ is sufficient, we do _not_ need to run cross-validation, but can again use the OOB error (works bost for bagging and random forests).
