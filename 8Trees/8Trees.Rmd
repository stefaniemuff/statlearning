...
* For $\alpha=0$ we get $T_0$ and as $\alpha$ increases we get shorter and shorter trees.
...
Interestingly, it seems like a tree with 5 leaves performs best - which corresponds to the original choice.
...
```{r headInjury7, echo=TRUE, fig.width=9, fig.height=5,fig.align = "center",out.width='90%'}
set.seed(33)
N=dim(headInjury2)[1]
...
* Originally, @Breiman1996 suggested to prune each tree, but later research has found that it is better to leave the trees at maximal size (a bushy tree), to make the trees as different from each other as possible.
...
* We use a subset of the observations in each bootstrap sample. Recall (module 5) that the probability that an observation is in the bootstrap sample is approximately $1-e^{-1}$=`r 1-exp(-1)` ($\approx 2/3$).
...
Todo (Exercise): Make a similar example, and let students plot the error rate against ntree (e.g. from 1 to 500, like figure 8.8).
...
* Drawback: It becomes difficult to interpret the results. Instead of having one tree, the resulting model consists of many trees. So, which predictors are (how) important?

<!-- Bagging is an example of an _ensemble method_, so is boosting and random forests (to come next). For all of these methods many trees are grown and combined, and the predictive power can be highly improved. However, this comes at a cost of interpretability.  -->
...
`varImpPlot` (or `importance`) in `randomForest` with `type=2`.
...
* Not optimal, as we get $B$ trees that are highly correlated. We don't get a large reduction in variance by averaging $\hat{f}^{*b}(x)$ when the correlation between the trees is high. In the previous section we actually even saw a (marginal) decrease in the predictive performance for the bagged tree compared to the pruned tree and the full tree.
...
* _Random forests_ provide an improvement over bagged trees by a small tweak that decorrelates the trees. This
...
a tree is considered, a _random selection_ of $m$ predictors is
...
If $B$ is sufficiently large (three times the number needed for the random forest to stabilize), the OOB error estimate is equvalent to LOOCV [see @Efron_Hastie2016, p. 330].
...
```{r iris, echo=FALSE, fig.width=8, fig.height=3.5,fig.align = "center",out.width='70%'}
...
```{r iris2, echo=FALSE, fig.width=8, fig.height=3.5,fig.align = "center",out.width='70%'}
var_importance <- data_frame(variable=setdiff(colnames(iris), "Species"),
                             importance=as.vector(importance(rf_out)))
var_importance <- arrange(var_importance, desc(importance))
var_importance$variable <- factor(var_importance$variable, levels=var_importance$variable)
...
\normalsize
Remember the data set: The aim is to predict the median value of woner occupied homes (in 1000\$)
...
```{r boston1, echo=TRUE, fig.width=8, fig.height=6,fig.align = "center",out.width='70%'}
plot(tree.boston)
text(tree.boston,pretty=0)
```

\small
Remember: the `tree()` function has a built-in default stopping criterion to decide when to stop enlarging the tree. You can change this with the `control` option, for example by setting `control = tree.control(mincut = 2, minsize = 4, mindev = 0.001)`. You can then continue with the following steps from there.
...
Let's changing from bagging to a random forest (n.b., why are we now speaking of a _forst_ and no longer of a _tree_?), using 6 randomly selected predictors for each tree:
...
First for model with $\lambda=0.001$ (default), then with $\lambda=0.2$, to calculae the MSE on the test set. We could have done cross-validation to find the best $\lambda$ over a grid.
...
I thank Thea Roksv{\aa}g, who developed the set of slides that I modified to get this set of slides. In addition, Mette Langaas and Julia Debik have contributed to the original version.
