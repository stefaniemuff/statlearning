...
* For $K$ classes $\mathcal{C} = \{c_1, c_2,\ldots, c_K\}$, with elements numbered as $1, 2, ..., K$, the probability that a new observation $x_0$ belongs to class $k$ is\footnote{This is the conditional probability: the probability that $Y=k$ given the observation $x_0$.}
...
To understand what test error means, we need to understand the _training error_ first.
...
$$\frac{1}{n}\sum_{i=1}^n \text{I}(y_i \neq \hat{y}_i), $$ 
where  I is the indicator function (to give our 0/1 loss) which is defined as: $$\text{I}(a\neq\hat{a}) = \begin{cases} 1 \text{ if } a \neq \hat{a} \\ 0 \text{ else } \end{cases}$$
...
knn_ds = data.frame(X1 = 1:60, X2 = misclas)
...
* The **diagnostic paradigm**: We focus on _directly_ estimating the posterior distribution for the classes $\text{Pr}(Y=k \mid X=x)$.
...
The $K$-nearest neighbour classifier estimates $\text{Pr}(Y=k \mid X=x)$ and classifies a new observation based on this estimated probability
...
A new observation $x_0$ will be classified to $A$ if $\text{Pr}(Y=A | X=x_0) > 0.5$ and to class $B$ otherwise.
...
* The **diagnostic paradigm**: We focus on _directly_ estimating the posterior distribution for the classes $\text{Pr}(Y=k \mid X=x)$. We have looked at the KNN-classifier in Part A.

Focus now is on _diagnostic paradigm_ = we estimates $\text{Pr}(Y=k \mid X=x)$ and classify a new observation based on this estimated probability.
...
\begin{align*}\log \frac{\text{Pr}(Y=0 |  X = x)}{\text{Pr}(Y=1 | X=x)} &= \log \frac{\pi_0}{\pi_1} + \log \frac{f_0(x)}{f_1(x)} \\ &= \log \frac{\pi_0}{\pi_1} -\frac{1}{2 \sigma^2}(x-\mu_0)^2 + \frac{1}{2 \sigma^2}(x-\mu_1)^2 \\ &= \log \frac{\pi_0}{\pi_1} - \frac{1}{2 \sigma^2} (x^2 - 2 x \mu_0 + \mu_0^2 - x^2 + 2 x \mu_1 - \mu_1^2) \\ &= \log \frac{\pi_0}{\pi_1} -\frac{1}{2 \sigma^2}(\mu_0^2 - \mu_1^2)+\frac{1}{\sigma^2}(\mu_0 - \mu_1) x \\ &= \alpha_0 + \alpha_1 x \end{align*}
...
* Logistic regression uses the diagnostic paradigm, and models the posterior distribution $\text{Pr}(Y=1|{\bf x})$.
