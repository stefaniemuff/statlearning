...
<!-- \frac{p_i}{1-p_i}=&\frac{\text{Pr}(Y_i=1|{\bf x}_i)}{\text{Pr}(Y_i=0|{\bf x}_i)}=\exp(\beta_0)\cdot \exp(\beta_1 x_{i1})\cdots\exp(\beta_p x_{ip}) -->
<!-- \end{align*} -->
...
* Caveat: We usually don't know the conditional distribution of $Y$ given $X$ for real data.

\vspace{2mm}

* We have discussed how to do it with logistic regression (for two categories).

\vspace{2mm}

* The $K$-nearest neighbor (KNN) classifier estimates this conditional distribution and chooses the most likely cateogry (Bayes classifier).\footnote{Attention!! $K$ refers to the number of neighbours used for the classifier, and \emph{not} to the number of classes!! The latter is assumed known.} 
...
* That means that $x_0$ is classified to the most occurring class among its neighbors ("majority vote")
...
```{r knnerror1,echo=FALSE,fig.width=6,fig.height=4,fig.align="center",out.width="90%"}
misclas=numeric(50)
n = dim(df)[1]
set.seed(9)
for(k in 1:50){
...
* The focus is on estimating the prior probabilities $\pi_k=\text{Pr}(Y=k)$ for the classes and the class conditional distributions $f_k(x)=\text{Pr}(X=x \mid Y=k)$. 
* We classify to the class with the maximal product $\pi_k f_k(x)$. In this paradigm, we need to model the pdf for each class. 
...
* the  _\textcolor{red}{prior}_ proability for class $k$ $\pi_k = \text{Pr}(Y=k)$ is the prior probability.
...
## Classification to the highest density
...
```{r gauss3, echo=FALSE, fig.width=4, fig.height=3,fig.align = "center",out.width='60%'}
library(ggplot2)
library(ggpubr)
x = seq(-6, 6, by = 0.01)
n = length(x)
...
XY = data.frame(x, y1, y2)
ggplot(XY)+geom_line(aes(x=x, y=0.5*y1), col="darkgreen")+geom_line(aes(x=x, y=0.5*y2), col="orange")+geom_vline(xintercept=0, linetype="dashed")+ annotate("text", x = 1.7, y = 0.175, parse=TRUE, label = as.character(expression(paste(pi[1], "=", pi[2], "=", 0.5))))+labs(y=NULL)
```
...
* Is it possible to construct a classifier and it has a lower error rate than the Bayes error rate?
...
* The Bayes decision boundary is at $x=0$, and we would make a (Bayes) error of `round(2*0.5*pnorm(0,2,1.5))`=`r round(2*0.5*pnorm(0,2,1.5),2)`.
...
Simulate data and use training data to estimate $\mu_k$ and $\hat\sigma^2$:
...
\vspace{2mm}
Todo: Verify that the following code will give you the training and test error rates:
...
This is a rather good performance, compared to the minimal Bayes error rate. But keep in mind that we LDA classifier relies on the Normal assumption, and that $\sigma_k=\sigma$ for all classes is assumed.
...
* Consider again our simulateion from a bivariate normal distribution with mean vectors $\mu_A  = (1, 1)^T$, $\mu_B = (3, 3)^T$, and covariance matrix $\Sigma_A = \Sigma_B = \begin{pmatrix} 2\hspace{2mm}   0 \\ 0 \hspace{2mm} 2 \end{pmatrix}$.  

* Aim: Use LDA to classify a new observation $x_0$ to class $A$ or $B$.


```{r synthAB2, echo=FALSE, fig.width=6, fig.height=4.5,fig.align = "center",out.width='60%'}
library(mvtnorm)
library(MASS)
library(ggplot2)
...
* Since we have bivariate normal class distributions with common covariance matrix, the optimal boundary is given by LDA, with boundary given at $\delta_A({\bf x})=\delta_B({\bf x})$.

<!-- where $\delta_A({\bf x})= {\bf x}^T \boldsymbol{\Sigma}^{-1}\boldsymbol\mu_A - \frac{1}{2}\boldsymbol\mu_A^T \boldsymbol{\Sigma}^{-1}\boldsymbol\mu_A + \log \pi_A$, and for $\delta_B({\bf x})$ with $\boldsymbol\mu_B$. -->
...
However, again, the Bayes boundary is usually not known, and we must estimate it from the data.
...
To obtain parameters estimates and an estimated decision boundary, we can use the `lda()` function in R.
...
```{r synthAB3, echo=FALSE, fig.width=6, fig.height=4.5,fig.align = "center",out.width='60%'}
...
ggplot(df, aes(x=X1, y=X2, color=class))+geom_point(size=3) + 
  ...
  geom_abline(intercept=koeff[1],slope=koeff[2],lty=2) 
```
...
```{r iris1, echo=FALSE, fig.width=5.5, fig.height=4,fig.align = "center",out.width='100%'}
...
```{r iris2, echo=FALSE, fig.width=5, fig.height=3.5,fig.align = "center",out.width='80%'}
...
irislda_plot = iris0_plot + geom_point(aes(x = Sepal.Width, y=Sepal.Length, 
                            colour=Species_lda), data=iris_lda_df, size=0.5)
irislda_plot
```
...
```{r iris3, echo=FALSE, fig.width=5, fig.height=3.5,fig.align = "center",out.width='80%'}
...
irisqda_plot = iris0_plot + geom_point(aes(x = Sepal.Width, y=Sepal.Length, 
                            colour=Species_qda), data=iris_qda_df, size=0.5)
irisqda_plot
```
...
* KNN is completely different, as it makes no assumptions about the decision boundary nor the distribution of the variables (nonparametric). It is expected to work better than LDA and logistic regression when boundary is very non-linear.
...
\vspace{2mm}
Todo: Include exercise(s) that compares LDA, QDA, logreg and KNN!
...
* **Aim**: To estimate the probability of a `chd` event for the observations in the test set. 

* To do this we can insert the estimated coefficient into the logistic equation, remembering that `famhist` is a categorical covariate, modeled by a dummy variable: 
...
with $\eta=\beta_0 + \beta_1 x_{\text{sbp}} + \beta_2 x_{\text{tobacco}} + \beta_3 x_\text{ldh} + \beta_4 \cdot x_{famhist} +  \beta_5 x_\text{obesity} + \beta_6 x_\text{alcohol} + \beta_7 x_{\text{age}}$.
...
```
The logistic model has correctly classified 118+41 times, and misclassified 27+45 times. The misclassification test error rate is thus:
$$\text{Test error} = \frac{27+45}{27+45+118+41} \approx 0.31$$
...
\scriptsize
```{r roc, echo=FALSE, fig.width=5, fig.height=4.5,fig.align = "center",out.width='80%'}
library(pROC)
