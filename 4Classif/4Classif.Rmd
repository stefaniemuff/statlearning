...
* We need more statistical theory than is presented in the textbook, which you find on these module slides.
...
* We assume that pairs of covariates and responses  $\{{\boldsymbol x}_i, y_i\}$ are measured independently of each other. Given $n$ such observation pairs, the likelihood function of a logistic regression model can be written as:
...
* **Caveat** for the Bayes classifier: we usually don't know the conditional distribution of $\text{Pr}(Y  |  X)$ is for real data.
...
* For $K=1$ the classification is made to the same class as the one nearest neighbor. As $K$ gets very large, the decision boundary tends towards a straight line (which is the Bayes boundary in this set-up).   

* Finding the _optimal value_ of $K$: try different values of $K$ and then test the predictive power of the different classifiers, for example by cross-validation (Module 5).


```{r knnerror1,echo=FALSE,fig.width=6,fig.height=4,fig.align="center",out.width="60%"}
misclas=numeric(50)
n = dim(df)[1]
set.seed(9)
for(k in 1:50){
...
than two response classes, because it also provides low-dimensional views of the data.
...
1. Would another division of the data into training and test set give the same conclusion (that LDA is better than QDA for this data set)? (A: Not necessarily, but probably.)
    + We will look into other choice than dividing into one training and one test set in Module 5 (cross-validation).
...
with $\eta=\beta_0 + \beta_1 \cdot x_{\text{sbp}} + \beta_2\cdot x_{\text{tobacco}} + \beta_3 \cdot x_\text{ldh} + \beta_4  \cdot x_{famhist} +  \beta_5\cdot x_\text{obesity} + \beta_6\cdot x_\text{alcohol} + \beta_7\cdot x_{\text{age}}$. 

\hspace{6mm} Remember that $x_{famhist} \in \{0,1\}$ is a binary covariate.

\vspace{2mm}

* We are interested in the **predictions for the test set** (although it is of course possible to calculate predictions for the training set, too). 
...
* Classifications are obtained by specifying a threshold value. We have here chosen a threshold value of 0.5. By using the `ifelse` function we specify that all probabilities larger than 0.5 are to be classified as 1, while the remaining probabilities are to be classified as 0.
