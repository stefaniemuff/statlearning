...
* Estimating the risk that someone will suffer from a heart disease or heart attack, given knowledge about condition, behaviour, age, or demographic, diet and clinical measurements. Here the _outcome is binary_ (yes, no) with both qualitative and quantitative input variables. 
...
The table below show the average percentage of words or characters in an email message, based on 4601 emails of which 1813 were classified as a spam.
...
Supervised learning examples (we will study):  
  ...
  * Classification trees, bagging, boosting (M8), K-nearest neighbor classifier (M4)
  * Support vector machines (M9)
  * Neural networks (M11)
  
$\rightarrow$ The majority of problems studied here fall in the supervised learning category
...
* It is an exciting research area, having important
...
**Aim**: _understand_ how the response variable is affected by the various predictors (covariates). 
...
* The $K$-nearest neighbour algorithm is an example of a non-parametric model. Used in classification, this algorithm predicts a class membership for a new observation by making a majority vote based on its $K$ nearest neighbours. We will discuss the $K$-nearest neighbour algorithm in Module 4.

(Todo: For the exercises make sure no KNN questions are in RecEx2 -- I moved all this to module 4, because it was otherwise resdundant. You can also move the respective questions to RecEx4, if that makes sense.)
...
Advantages | Disadvantages
------------------------------------ | ------------------------------------ 
Flexible: a large number of functional forms can be fitted|Can overfit the data\vspace{2mm}
No strong assumptions about the underlying function are made|Computationally more expensive as more parameters need to be estimated\vspace{2mm}
...
```{r data, echo=FALSE, fig.width=5, fig.height=3,fig.align = "center"}
library(ggplot2)
library(ggpubr)
set.seed(2)
...
```{r,echo=FALSE,fig.width=7,fig.height=4.5,fig.align="center",out.widht='80%'}
...
* Assume we have fitted a _regression_ curve $Y  = f(x) + \varepsilon$ to our training data, which consist of independent observation pairs $\{x_i, y_i\}$ for $i=1,..,n$. (Yes, only one covariate $x$.)
...
However, if we also assume that $X$ is a random variable, this is really 
 $\text{E}[(Y - \hat{f}(x_0))^2 \mid X=x_0]$ 

The **overall expected test MSE** can we then compute by averaging the expected test MSE over all possible values of $x_0$ (averaging with respect to frequency in test set),
...
* **Training set**: The observations used to fit the statistical model $\rightarrow$ Training error

* **Test sample**: new observations which were not used when fitting the model $\rightarrow$ Test error 
