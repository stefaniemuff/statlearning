...
By applying *statistical methods* on a *data set* (called the *training set*), we would like to 

* *draw conclusions* about the relations between the variables (_\textcolor{red}{inference}_) or 
* *find a predictive function* for new observations (_\textcolor{red}{predition}_). 
...
* Classification problem, where the response variable is categorical with classes \{0, 1, 2, ..., 9\}.
\vspace{1mm}
...
**Aim**: understand_how the response variable is affected by the various predictors (covariates). 
...
$$Y  = f(x) + \varepsilon$$ 
to our training data $\{x_i, y_i\}$ for $i=1,..,n$, and $\varepsilon$ is an unobserved random variable that adds random, uncorrelated noise with mean zero and constant variance $\sigma^2$. $\varepsilon$ is a substitute for all the unobserved variables that influence $Y$. 
