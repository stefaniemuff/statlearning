...
* A random vector $\boldsymbol{X}_{(p\times 1)}$ is a $p$-dimensional vector of random variables. 
...
The moments are important properties about the distribution of $\boldsymbol{X}$. We will look at:
...
**Q**: what are the univariate analog to this formula - that you studied in your first introductory course in statistics? What do you think happens if we look at $\text{E}(\boldsymbol{A}\boldsymbol{X}\boldsymbol{B})+\boldsymbol{d}$?
...
## Random vectors - Single-choice klicker exercise

\vspace{4mm}

In class - go to [https://app.klicker.uzh.ch/join/bkx](https://app.klicker.uzh.ch/join/bkx) to answer the questions on the following slides
