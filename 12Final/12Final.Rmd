...
* The aim is to _\textcolor{red}{explain}_ $y$ using known regressors. In this case, the ultimate aim is to find causal relationships.
...
**Example:** The South African heart disease data aims to identify important risk factors for coronary heart disease.
...
## Model selection with AIC, BIC, $C_p$, adjusted $R^2$
...
$\rightarrow$ Subset selection may lead to **biased parameter estimates**, thus **do not draw (biological, medical,..) conclusions** from models that were optimized for prediction, for example by AIC/AICc/BIC minimization!
...
Then you are basically free to do what you want. Your only aim is to minimize some sort of prediction error.
...
In such a situation, you should formulate a _\textcolor{red}{confirmatory model}_:
...
* Ideally, formulate _\textcolor{red}{only one}_ or a few model(s) _\textcolor{red}{before you start analysing your data}_.
...
$\rightarrow$ Two sorts of _\textcolor{red}{explanatory models/analyses}_:
...
Explanatory models to confirm hypotheses (so-called confirmatory models) have a long tradition medicine. In fact, the main conclusions in a study are only allowed to be drawn from the main model (which needs to be specified even before data are collected):
...
\centering
```{r pvaluesHist,fig.width=4, fig.height=4.5,out.width="5cm"}
...
**Thank you for attending this course  - good luck for the compulsory exercise 3 and let's hope we are soon back into normal teaching mode in the post-Covid-19 era!**
