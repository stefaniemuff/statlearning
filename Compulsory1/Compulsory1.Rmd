...
**The submission deadline is Thursday, 22. February 2021, 23:59h using Blackboard**
...
$f(\mathbf{x}) = \mathbf{x}^T\boldsymbol{\beta},$ where $\mathbf{x}$ and $\boldsymbol{\beta}$ are both vectors of length $p+1.$ 

We know that the OLS estimator in this case is $\hat{\boldsymbol{\beta}} = (\mathbf{X}^T\mathbf{X})^{-1}\mathbf{X}^T\mathbf{Y}.$ We look now at a competing estimator $\widetilde{\boldsymbol \beta} =(\mathbf{X}^T\mathbf{X}+\lambda {\bf I})^{-1}\mathbf{X}^T{\bf Y}$ (ridge regression estimator) where $\lambda\ge0$ is  a constant tuning parameter and controls the bias-variance trade-off. Observe that for $\lambda = 0$ the ridge regression estimator is equivalent to $\hat{\boldsymbol{\beta}}.$
...
Answer the questions by fitting appropriate models (1P for the correct model) and giving the correct reasoning of your answer (1P per question). Important: Carefully think what ``appropriate model'' means. Do not throw away information in the data...
