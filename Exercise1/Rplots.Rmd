...
For each of the plots (scatter plot, histogram, boxplot, area chart, heat map, correlogram) _explain what you see (including what is on the x- and y-axis) and try to transform what you see into insight about the data_. All except the correlogram use `ggplot2` for plotting. If you want to read more about the idea behind `ggplot2` (grammar of graphics) Chapter 3 of [R for Data Science](https://r4ds.had.co.nz/data-visualisation.html) is a good read. Other resources are:
...
```{r, echo=FALSE, results='asis', eval=showsol}
cat("**Solution:** 
Visualization of the data. Shows the values of the different covariates (-1 to 3) for the different car models. Note that the covariates were centered and standardized to have a mean of 0 and a standard deviation of 1, thus it is possible to compare them more easily.
")
```
