...
\NormalTok{  dat }\OtherTok{=} \FunctionTok{rbind}\NormalTok{(dat, }\FunctionTok{data.frame}\NormalTok{(}\AttributeTok{horsepower =}\NormalTok{ ds[tr,}\DecValTok{1}\NormalTok{], }\AttributeTok{mpg =}\NormalTok{ mod}\SpecialCharTok{$}\NormalTok{fit, }
...
\NormalTok{dat }\OtherTok{=} \FunctionTok{data.frame}\NormalTok{(}\AttributeTok{origin =}\NormalTok{ new, }\AttributeTok{mpg =}\NormalTok{ pred}\SpecialCharTok{$}\NormalTok{fit, }
...
\FunctionTok{ggplot}\NormalTok{(dat, }\FunctionTok{aes}\NormalTok{(}\AttributeTok{x=}\NormalTok{origin, }\AttributeTok{y=}\NormalTok{mpg)) }\SpecialCharTok{+} \FunctionTok{geom\_point}\NormalTok{() }\SpecialCharTok{+} 
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Anyway, if we now use our model matrix to fit a linear regression model (excluding the intercept {-}1, because the first column of X already contains only 1\textquotesingle{}s and thus encodes for an intercept), we can obtain predicted values for yhat}
\NormalTok{myhat }\OtherTok{=} \FunctionTok{lm}\NormalTok{(wage}\SpecialCharTok{\textasciitilde{}}\NormalTok{X}\DecValTok{{-}1}\NormalTok{)}\SpecialCharTok{$}\NormalTok{fit}
...
\NormalTok{fitgam}\OtherTok{=}\FunctionTok{gam}\NormalTok{(mpg}\SpecialCharTok{\textasciitilde{}}\FunctionTok{bs}\NormalTok{(displacement, }\AttributeTok{knots=}\FunctionTok{c}\NormalTok{(}\DecValTok{290}\NormalTok{))}\SpecialCharTok{+}
