...
\normalsize
* All lines going into the output node signifies that we multiply the covariate values in the input nodes with the _\textcolor{red}{weights}_ (regression coefficients), and then sum. 
* This sum can be sent through a so-called _\textcolor{red}{activition function}_ (here just the identity function).
...
3.  Replace _minimizing_ the loss function (RSS) by _\textcolor{red}{more general minization procedures}_ that work also when the loss function does not have a closed form. In particular, use the the _gradient descent algorithm_.
...
* For neural networks the negative of the multinomial loglikelihood is a scaled version of the _\textcolor{red}{categorical cross-entropy loss}_
...
```{r}
library(caret)
fit=multinom(species~  -1 + .,family=multinomial, data=iris_train, trace=FALSE)
coef(fit)
fit$wts
```
...
* _\textcolor{red}{Backpropagation}_ is a simple and inexpensive way _to calculate the gradient_.
...
* The _\textcolor{red}{chain rule}_ is used to compute derivatives of functions of other functions where the derivatives are known. This is efficiently done with backpropagation.
...
* _\textcolor{red}{Dataset augmentation}_: Adding fake data to the dataset, in order that the trained model will generalize better. For some learning tasks it is straightforward to create fake data. For image data this can be done by rotating and scaling the images.
...
* The success of deep learning is dependent upon the breakthroughts 
    ...
    +  _\textcolor{red}{datasets}_ and benchmarks (internet/tech data).
