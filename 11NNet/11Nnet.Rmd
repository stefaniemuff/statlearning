...
The _\textcolor{red}{universal approximation theorem}_\footnote{Goodfellow et al 2016, p.194, https://www.deeplearningbook.org} says that a feedforward network with
\vspace{2mm}
...
$\rightarrow$ Cross-validation! (Time-consumring, so only results shown below.)
...
* Replacing sigmoid with ReLU is reported to be one of the major changes that have improved the performance of the feedforward networks\footnote{Goodfellow et al, page 226}.

* Easy to use with gradient descent -- even though the function is not differentiable at 0. As we will touch upon later, we don't expect to train a network until the gradient is 0. The derivative from the left at 0 is 0, and the derivative from the right is 1\footnote{See Goodfellow et al, p.192 for a discussion on this topic.}.
...
According to @goodfellow, page 191, hidden unit design is an _active area of research._
...
* The choice of loss function is closely related to the output layer activation function. 
...
* The gradient as _an average over many individual gradients_, and think of this as an estimator for an expectation. This expectation can be approximated by the average gradient over just a _mini-batch_ (random sample) of the observations.

* Idea: the optimizer will converge much faster if they can rapidly compute approximate estimates of the gradient, instead of slowly computing the exact gradient (using all training data).

* Mini-batches may be processed _in parallel_, and the batch size is often a power of 2 (32 or 256).

* Additional benefit: It also turns out that small batches also serves as a regularization effect maybe due to the variability they bring to the optimization process.
...
**Special case**: Stochastic gradient descent (SGD) for only one sample (mini-batch size 1).  Mini-batch SGD is a compromise between SGD (one sample per iteration) and full gradient descent (full dataset per iteration)
...
* _\textcolor{red}{Backpropagation}_ is a simple and inexpensive way to calculate the gradient.
...
* Mathematical details: @goodfellow Section 6.5 (pages 204-238).
...
* AdaGrad: individually adapt the learning rates ($\lambda$) of all model parameters. The
parameters with the largest partial derivative of the loss have a correspondingly
rapid decrease in their learning rate. Nice properties when optimization is convec.
...
* _Weight decay_: In neural networks this means adding a $L_2$-penalty to the loss function (see chapter 7.1.1 in Goodfellow et al), and is available in the `nnet` R package.

\vspace{2mm}

* _Dataset augmentation_: Adding fake data to the dataset, in order that the trained model will generalize better. For some learning task it is straightforward to create the new fake data. For image data this can be done by rotating and scaling the images.
...
**Hyperparameters**: The network architecture, the number of batches to run before terminating the optimzation, the drop-out rate.
...
* The success of deep learning is dependent upon the breakthroughts in hardware development, expecially with faster CPUs and massively parallell graphical processing units (GPUs). 
...
We are using a `layer_dense` (fully connected) which expects an input tensor of rank equal to two `(sample, features)` where each `sample` should contain `28*28=784` pixels. Adding a bias term (intercept) is default for `layer_dense`.
...
* We next need to choose the loss function we will use, which is cross-entropy, and then the version of the optimizer. Here this is RMSprop. 

* Finally, which measure (metrics) do we want to monitor in our training phase? Here we choose accuracy (=percentage correctly classified).
...
* @kerasR
* @goodfellow (to be used in IT3030) <https://www.deeplearningbook.org/>
